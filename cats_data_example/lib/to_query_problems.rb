...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    FROM
      toys AS toys1 
    JOIN 
      toys AS toys2 ON toys1.price = toys2.price
    WHERE 
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT
      cats2.name, cats2.breed
    FROM
      cats AS cats1
    JOIN
      cats AS cats2 ON cats1.breed = cats2.breed
    WHERE
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      cats2.name, toys.name 
    FROM cats AS cats1
    JOIN cattoys ON cats1.id = cattoys.cat_id
    JOIN toys AS toys ON toys.id = cattoys.toy_id
    JOIN cattoys AS cattoys2 ON toys.id = cattoys2.toy_id
    JOIN cats AS cats2 ON cats2.id = cattoys2.cat_id
